...
async function extractOfficeText(buffer: Buffer, extension: string) {
  ...
  const chunks: string[] = [];
  for (const path of xmlPaths.slice(0, 60)) {
    const file = zip.files[path];
    if (file) chunks.push(await file.async("text"));
  }
...
async function extractZipInspection(buffer: Buffer) {
  ...
  const list = entries.slice(0, 100).map((file) => `- ${file.name}`).join("\n");
  const samples: string[] = [];

  for (const file of entries.slice(0, 16)) {
    if (!/\.(txt|md|csv|json|html|css|js|ts|tsx|jsx)$/i.test(file.name)) continue;
    samples.push(`\n--- ${file.name} ---\n${(await file.async("text")).slice(0, 3000)}`);
  }
...
function extractPdfText(buffer: Buffer) {
  ...
  return cleanText(strings || "PDF text extraction is limited until pdf-parse can be installed. Use any visible text and file context available.");
}
...
export async function processUploadedChatFile(file: UploadedChatFile): Promise<ProcessedChatFile> {
  ...
  if (isImage) {
    return {
      ...
      extractedText: "Image uploaded. Inspect the image visually, read visible text, and answer the user from the image.",
